...
MQTT_PORT = 1883
MQTT_TOPIC_PUB = "mc/sensor/pulso"
    ...
    try:
        ...
        if valor_pulso > umbral_pulso:
            if time.ticks_diff(tiempo_actual, ultimo_latido) > 500:  # Evitar detección múltiple en 500ms
                intervalo = time.ticks_diff(tiempo_actual, ultimo_latido) / 1000 #Tiempo entre dos latidos en segundos.
                bpm = int(60 / intervalo) #Convierte ese tiempo en latidos por minuto (BPM).
                ultimo_latido = tiempo_actual

                ventana_bpm.append(bpm) # Agrega el nuevo valor de BPM a la lista de mediciones recientes.
                if len(ventana_bpm) > tamanio_ventana: #Si la lista tiene más valores que el tamaño del filtro (5)
